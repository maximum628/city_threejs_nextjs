...
export class Viewer {
  ...
  constructor (el, options) {
    ...
    this.state = {
      ...
      bgColor1: options.background,//'#ffffff',
      bgColor2: '#353535'
    };
    ...
    const fov = options.preset === Preset.ASSET_GENERATOR
      ? 0.8 * 180 / Math.PI
      : 60;
    const el_width = el.getAttribute('width');
    const el_height = el.getAttribute('height');
    this.defaultCamera = new PerspectiveCamera( fov, el_width / el_height, 0.01, 10000 );
    this.activeCamera = this.defaultCamera;
    this.activeCamera.position.set( 30, 60, 130 );
    this.activeCamera.lookAt( 0, 1, 0 );
    ...
    this.renderer.setSize( el_width, el_height );
  ...
  setContent ( object, clips ) {
    ...
    // const material_loader = new MaterialLoader();
    this.content.traverse((node) => {
      if (node.isLight) {
        const sphereSize = 1;
        const pointLightHelper = new PointLightHelper( node, sphereSize );
        this.scene.add( pointLightHelper );
        this.state.addLights = false;
      } else if (node.isMesh) {
        // console.log( node );
        // console.log( material_loader.parse(node.material) );
        ...
        const particleUniforms = {
					...
					'cameraConstant': { value: this.getCameraConstant( this.defaultCamera ) },
					'density': { value: 0.0 }
				};
  ...
  updateEnvironment () {
    ...
    this.getCubeMapTexture( environment ).then(( { envMap } ) => {
      ...
      this.scene.background = this.state.background ? this.options.background : null;
